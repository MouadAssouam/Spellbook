...
/**
 * Metadata about the live API response that produced a GeneratedSpell.
 * Lets the CLI / UI show "actually worked" status without re-fetching.
 */
...
export interface MagicResult {
    ...
    error?: string;
    warnings?: string[];
    /** Details of the live response that verified this spell. */
...
/**
 * Converts a GeneratedSpell (from magicFromUrl) into a full Spell
 * suitable for generateMCPServerV2().
 *
 * This is the bridge between "URL -> tested result" and "generate code".
 * Callers (CLI, VS Code) should use this instead of hand-building the Spell,
 * so the mapping stays consistent.
 *
 * @param generated - The output of a successful magicFromUrl() call
 * @param overrides - Optional overrides for name/description
 * @returns A validated Spell ready for code generation
 */
export function magicToSpell(
    generated: GeneratedSpell,
    overrides?: { name?: string; description?: string }
): Spell {
    const name = overrides?.name || generated.name;
    const tool = generated.tool;

    const inputSchema = tool.parameters.length > 0
        ? {
            type: 'object' as const,
            properties: Object.fromEntries(
                tool.parameters.map(p => [p.name, { type: p.type }])
            ),
    ...
    return {
        ...
        tools: [{
            ...
            action: {
                ...
                config: {
                    url: tool.url,
                    method: tool.method.toUpperCase() as 'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE'
                }
            }
        }]
    };
